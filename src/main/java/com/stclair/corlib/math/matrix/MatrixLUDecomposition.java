...
public class MatrixLUDecomposition {
    ...
    <T> int firstSolveableRow(Array2D<T> members, int column, OperationStrategy<T> op) {
        for (int row = column; row < members.getHeight(); row++) {
            if (! op.isZero(members.get(column, row))) {
                return row;
            }
        }
